...
namespace ServiceAgence.SQL
{
    class TraitementsSQL
    {
        public static string DBStr(object valeur, bool trim = true)
        {
            if (object.ReferenceEquals(valeur, System.DBNull.Value)) return "";
            if (string.IsNullOrEmpty(Convert.ToString(valeur))) return "";

            if (trim) return Convert.ToString(valeur).Trim();
        ...
        {
            if (object.ReferenceEquals(valeur, System.DBNull.Value)) return 0;

            double dbl = 0;
            string strDbl = null;

            strDbl = Convert.ToString(valeur);
            if (double.TryParse(strDbl, out dbl)) return dbl;
        ...
        public static int DBInt(object valeur)
        {
            ...
            if (int.TryParse(strInt, out i)) return i;
        ...
        {
            if (object.ReferenceEquals(valeur, System.DBNull.Value)) return false;
        ...
        {
            if (object.ReferenceEquals(valeur, System.DBNull.Value)) return null;
        ...
        public static string FSQL<TSelf>(TSelf valeur)
        {
            ...
            if (valeur == null) tValeur = GetDeclaredType(valeur);
            else tValeur = valeur.GetType();
        ...
        private static string FSQL(object valeur, Type tValeur)
        {
            ...
            else if (tValeur == typeof(string) || tValeur == typeof(char)
                || tValeur == typeof(char?))
            {
